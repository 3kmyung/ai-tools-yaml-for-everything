...
| `action` | `model` 컴포넌트만, 스키마 기본값과 다르게 고정할 `action` 필드와 `params`를 `이름: 값`으로 |
| 출처 | 파일 경로나 URL |

#### 서비스

```
| 워크플로 | 흐름 | `title` | `description` | 입력 | 출력 | 주 모델 | `실행`에 쓸 입력 | 라이선스 |
| :---: | :---: | --- | --- | :---: | :---: | :---: | --- | --- |
```
...
| :---: | --- |
| `0` | 출력된 이름 그대로 사용 |
| `3` | 출력을 보여주고 변종 토큰(Token) 혹은 기존 서비스 수정 여부 질문 |
| `4` | 주 모델 체크포인트 질문 |
...
| 컴포넌트 `runtime.type` | | | `virtualenv` |
| 컴포넌트 `runtime.path` | | | `.venv/<컴포넌트 id>` |
