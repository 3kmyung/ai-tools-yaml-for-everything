...
**Goal:** Build `ui-house-style` and `model-showcase` so that `releases/speaker-diarization-vibevoice` can be generated from a model name rather than hand-written.

**Architecture:** Correct the reference UI at `releases/youtube-to-playlist-video/ui/` first, because the rules documents describe it and the acceptance check compares generated output against it. Then write the two skills as prose whose factual claims are checked against that corrected code by a script. Then generate the example and iterate on the skills — never on the example.
...
- Line endings are LF. `core.autocrlf=input` is set globally and locally; there is no `.gitattributes`. Verify with a byte-exact method — `od -An -t x1 <file> | tr ' ' '\n' | grep -c '^0d$'`, or `tr -cd '\r' < <file> | wc -c` — and not with `grep -c $'\r'`, which in a nested command substitution under this shell degenerates to an empty pattern that matches every line and reports a CR count equal to the line count for files that contain none. That artefact briefly convinced the controller this repository was CRLF throughout; it is not.
...
### Repository — `releases/youtube-to-playlist-video/`
...
| File | Action | Responsibility |
|---|---|---|
| `releases/speaker-diarization-vibevoice/**` | create | generated by `model-showcase` in Task 13 |
...
- Modify: `releases/youtube-to-playlist-video/.gitignore`
...
node .claude/skills/model-showcase/assets/ui-check.mjs \
  releases/youtube-to-playlist-video/ui
```
...
Add to `releases/youtube-to-playlist-video/.gitignore`:
...
```bash
git add releases/youtube-to-playlist-video/.gitignore
...
**Files:**
- Modify: `releases/youtube-to-playlist-video/ui/styles/base.css:8-11`
- Modify: `releases/youtube-to-playlist-video/ui/styles/components.css:279,337,341,461`
- Modify: `releases/youtube-to-playlist-video/ui/styles/layout.css:141`
...
```bash
git add releases/youtube-to-playlist-video/ui/styles
git commit -F - <<'EOF'
...
**Files:**
- Modify: `releases/youtube-to-playlist-video/ui/styles/base.css:55-58`
- Modify: `releases/youtube-to-playlist-video/ui/styles/components.css`
- Test: `.claude/skills/model-showcase/assets/checks.js`
...
**Files:**
- Modify: `releases/youtube-to-playlist-video/ui/styles/base.css`
- Test: `.claude/skills/model-showcase/assets/checks.js`
...
```bash
git add releases/youtube-to-playlist-video/ui/styles/base.css
git commit -F - <<'EOF'
...
All paths are under `releases/youtube-to-playlist-video/`.
...
```bash
git add releases/youtube-to-playlist-video/ui
git commit -F - <<'EOF'
...
**Files:**
- Modify: `releases/youtube-to-playlist-video/ui/styles/layout.css`
- Modify: `releases/youtube-to-playlist-video/ui/styles/components.css`
- Test: `.claude/skills/model-showcase/assets/checks.js`
...
for size in "1440 900" "800 900" "390 844"; do
  set -- $size
  node .claude/skills/model-showcase/assets/ui-check.mjs \
    releases/youtube-to-playlist-video/ui --width=$1 --height=$2
done
```
...
**Files:**
- Modify: `releases/youtube-to-playlist-video/ui/src/icons.js`
- Modify: `releases/youtube-to-playlist-video/ui/styles/components.css` (`.icon` stroke width)
- Test: `.claude/skills/model-showcase/assets/checks.js`
...
**Files:**
- Create: `releases/speaker-diarization-vibevoice/model-compose.yml`
- Create: `releases/speaker-diarization-vibevoice/ui/**`
- Create: `releases/speaker-diarization-vibevoice/README.md`, `README.ko.md`, `README.zh-cn.md`
- Create: `releases/speaker-diarization-vibevoice/.gitignore`
...
for size in "1440 900" "800 900" "390 844"; do
  set -- $size
  node .claude/skills/model-showcase/assets/ui-check.mjs \
    releases/speaker-diarization-vibevoice/ui --width=$1 --height=$2
done
```
...
Place the generated `ui/` beside `releases/youtube-to-playlist-video/ui/` and confirm each item:
...
```bash
rm -rf releases/speaker-diarization-vibevoice
```
...
```markdown
- [speaker-diarization-vibevoice](./showcase/speaker-diarization-vibevoice/) — Hour-long transcription with speakers and timestamps via VibeVoice-ASR
```

- [ ] **Step 7: Commit**

```bash
git add releases/speaker-diarization-vibevoice examples/README.md
git commit -F - <<'EOF'
Add the `speaker-diarization-vibevoice` showcase
...
**Files:**
- Modify: `releases/speaker-diarization-vibevoice/ui/fixture.json`
- Modify: `releases/speaker-diarization-vibevoice/README.md`, `README.ko.md`, `README.zh-cn.md` (any figure marked `needs verification`)
...
```bash
git add releases/speaker-diarization-vibevoice
git commit -F - <<'EOF'
Settle the figures `speaker-diarization-vibevoice` could not source
...
```bash
node .claude/skills/model-showcase/assets/ui-check.mjs \
  releases/youtube-to-playlist-video/ui --width=390 --height=844
```
