...
Two files hold the nineteen checks between them, and each has a different owner and a
different lifecycle.

`assets/checks.js` is the current, versioned source of truth for the thirteen checks that
...
| Category colour | every pair of `--category-1` through `--category-7` sits a CIEDE2000 of at least 20 apart |
...
| 18 | every pair of `--category-*` tokens clears a CIEDE2000 of 20 | `assets/checks.js` | identical everywhere |
| 19 | no border or outline width is a literal length | `assets/checks.js` | identical everywhere |
