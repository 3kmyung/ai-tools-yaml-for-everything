...
같은 작업을 로컬(Local) 모델(Model)과 클라우드 서비스(Cloud Service)로 비교하는 포스트 6개다.

| 포스트 | 내용 |
| :---: | --- |
| 1 | 두 결과물을 비교하게 만드는 캡션, 로컬과 클라우드 서비스의 결과물 첨부 |
| 2 | 클라우드 서비스가 아닌 로컬 모델만 소개하는 산문, 핵심 장점 체크리스트 한 줄 |
| 3 | 이것으로 무엇을 할 수 있는지, 독자가 체감하는 장점 |
| 4 | `write-readme`가 찍은 결과물 GIF |
...
[<로컬 결과물 경로>](<로컬 결과물 경로>)
[<클라우드 서비스 결과물 경로>](<클라우드 서비스 결과물 경로>)
